...
async function testMQTTAPI() {
    ...
    try {
        ...
        // 2. 测试MQTT状态
        console.log('\n📡 2. 查询MQTT状态...');
        const statusResponse = await axios.get(`${baseUrl}/api/mqtt/status`, { headers });
        ...
        // 3. 测试设备列表
        console.log('\n📱 3. 查询MQTT设备列表...');
        const devicesResponse = await axios.get(`${baseUrl}/api/mqtt/devices`, { headers });
        console.log(`✅ 查询到 ${devicesResponse.data.devices.length} 个安卓设备`);
        ...
        const createResponse = await axios.post(`${baseUrl}/api/devices`, testDevice, { headers });
        ...
        try {
            const pushResponse = await axios.post(`${baseUrl}/api/mqtt/push`, pushData, { headers });
        ...
        try {
            const commandResponse = await axios.post(`${baseUrl}/api/mqtt/command`, commandData, { headers });
        ...
        // 7. 清理测试数据
        console.log('\n🧹 7. 清理测试设备...');
        await axios.delete(`${baseUrl}/api/devices/${createResponse.data.device._id}`, { headers });
        console.log('✅ 测试设备已删除');
